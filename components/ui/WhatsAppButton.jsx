...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    const onScroll = () => setScrollBtnVisible(window.scrollY > SCROLL_THRESHOLD);
    onScroll();
  ...
  return (
    <a
      ...
      className={`group fixed right-6 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl ${
        scrollBtnVisible ? "bottom-20" : "bottom-6"
      }`}
    >
      <WhatsAppLogo className="h-9 w-9" />

      <span className="pointer-events-none absolute right-full top-1/2 mr-3 -translate-y-1/2 translate-x-2 whitespace-nowrap rounded-md bg-[#25D366] px-3 py-1.5 text-xs font-semibold text-cream opacity-0 shadow-lg transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100">
        Connect on WhatsApp
